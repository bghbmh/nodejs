...
export class UploadFiles {
    ...
    create = (args) => {
		...
		console.log("local00000000 - ", this.handler.multiple)
		...
		console.log("testFiles - ", localData)
	...
	async makeFileSomeThing(file, url, cal ){
		...
		//console.log("makeFileSomeThing 000 - ", this )
		reader.addEventListener("load", testLoad);

		function testLoad(){
			newFileList.items.add(new File([blob], file.name, { type: file.type, } ));

			//console.log("testLoad 111 - ", this )

			// this.selectedFiles.push(newFileList.files[0]);
			// this.objectURLs.set(newFileList.files[0], window.URL.createObjectURL(newFileList.files[0])); 

			cal( newFileList.files[0], window.URL.createObjectURL(newFileList.files[0]))
		}
		...
		// }
		let aa = reader.readAsDataURL(blob);
	}
